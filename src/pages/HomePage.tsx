...
export function HomePage() {
  ...
  return <><AppHeader context="从模糊想法，到清晰方向"/><main className={styles.home}>
    ...
    <section className={styles.entry}><Textarea label="先说说，你在想什么？" value={idea} onChange={e=>setIdea(e.target.value)} placeholder={copy.placeholder}/><div className={styles.actions}><small>不必完整，也不必知道专业术语。</small><Button variant="primary" onClick={()=>setDialog(true)}>开始探索 <ArrowRight size={15}/></Button></div><Button variant="ghost" onClick={() => { void navigate('/workspace/demo'); }}>先打开示例看看</Button></section>
    <section className={styles.recent}><div><small>继续一段探索</small><h2>研究型问答的可靠性</h2><p>示例项目 · 2 条探索分支</p></div><Button onClick={() => { void navigate('/workspace/demo'); }}>继续 <ArrowRight size={15}/></Button></section>

...
export function SessionSidebar({ activeId, refreshKey = 0 }: { activeId?: string; refreshKey?: number }) {
  ...
  return <aside className={styles.sidebar}>
    <div className={styles.brand}>IdeaScope</div>
    <button className={styles.newButton} onClick={() => void create()}><Plus size={16} />新建探索</button>
    <p className={styles.label}>探索会话</p>
    <nav>{items.map((item) => <div key={item.id} className={`${styles.item} ${item.id === activeId ? styles.active : ""}`}>

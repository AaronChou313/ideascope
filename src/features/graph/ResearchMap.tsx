...
export function ResearchMap({ branch, selectedId, contextNodeId, onSelect, onViewChange }: { branch: Branch; selectedId: string | null; contextNodeId: string | null; onSelect: (node: GraphNode) => void; onViewChange: (positions: PositionMap, viewport?: Viewport) => void }) {
  ...
  useEffect(() => { let live = true; void layoutGraph(branch.graph.nodes, branch.graph.edges, branch.view).then((value) => { if (!live) return; setPositions(value); if (Object.keys(branch.view.positions).length !== value.size) onViewChange(value); }); return () => { live = false; }; }, [branch.graph.nodes, branch.graph.edges, branch.view, onViewChange]);
  ...
  return <div className={styles.map}>
    ...
    <Button className={styles.arrange} disabled={arranging} title="重新整理当前研究地图" onClick={() => void arrange()}>{arranging ? "整理中…" : "整理布局"}</Button>
    <div className={styles.guides}>{[...new Set(branch.graph.nodes.map(({ depth }) => depth))].sort((a, b) => a - b).map((depth) => <span key={depth} style={{ left: depth * 350 + 24 }}>{depth === 0 ? "核心问题" : depth === 1 ? "主要路线" : depth === 2 ? "关键问题" : `深入探索 · ${depth - 2}`}</span>)}</div>
    <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} fitView={!hasSavedViewport} defaultViewport={branch.view.viewport} fitViewOptions={{ padding: .22 }} minZoom={.28} maxZoom={1.7} nodesDraggable={false} onNodeClick={(_, item) => onSelect(item.data.node as GraphNode)} onMoveEnd={(_, viewport) => onViewChange(positions, viewport)}><Background color="#d7d9dd" gap={20} size={.65}/><Controls showInteractive={false}/></ReactFlow>
  </div>;
}

...
缓存键必须覆盖来源、关键词、筛选、排序、选择字段和关键词语言。它不包含凭证。SearchRecord 与归一化 Paper 已写入本机 Dexie；缓存命中与有效期策略仍留在后续阶段接入。
...
检索结果只证明“来源返回了这些记录”，不证明论文支持某个研究判断。DOI、arXiv base/version 与 OpenAlex ID 会先规范化。精确 ID 重复可复用已有 Paper；题名、首位作者和相近年份只产生人工审阅候选。用户可保留独立记录或显式关联为不同版本，系统不会静默合并。Evidence 必须引用已保存 Paper，已存在的证据 ID 不允许用不同内容原地覆盖。

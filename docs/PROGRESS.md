...
当前状态：**v0.6.16 已修复真实 Workspace 标题栏折叠按钮与标题重叠；继续等待用户稳定性确认，不进入 v0.7.0。**
...
| v0.6.13    | 已完成 / 稳定性待确认 | 画布 viewport 回写隔离、异常视口自愈、空位置保护与显式“找回地图”恢复入口                                      |
| v0.6.14    | 已完成 / 稳定性待确认 | 设置移入会话侧栏、会话导入与对话搜索、节点搜索渐进展开、设置控件与全局备份层级统一                              |
| v0.6.15    | 已完成 / 稳定性待确认 | 侧栏折叠操作移入顶栏、单会话导入导出图标语义修正、多 Provider 保存/切换/删除与会话级密钥隔离                 |
| v0.6.16    | 已完成 / 稳定性待确认 | 补齐正式 Workspace 根作用域，恢复标题栏横向对齐，并增加按钮与标题不重叠的浏览器回归检查                    |

## v0.6.16 Workspace 标题栏对齐修复

实施日期：2026-09-11。正式 Workspace 分支遗漏 `workspace-root-shell` 根类，导致 v0.6.15 的标题栏 flex 规则只在空状态分支生效；真实会话中的折叠按钮因此与标题按块流叠放。本次补齐统一根作用域并整理标题栏结构，展开与收起状态下按钮均位于标题左侧且垂直居中。Playwright 增加按钮右边界不得越过标题左边界的几何回归断言，并在真实 1294px 页面检查两种状态。

## v0.6.15 工作台工具栏与多 Provider

实施日期：2026-09-11。本阶段只处理用户反馈的工作台按钮遮挡、会话导入导出图标语义和多模型 Provider 配置，不修改研究 Agent、图谱语义或文献检索流程。

工作台：左右侧栏的折叠/展开按钮从画布浮层移入中央顶栏，展开和收起状态均可操作，不再覆盖“找回地图”“整理布局”等画布控件。当前会话导出改用向上分享箭头；侧栏单会话导入改用从外部文件导入图标，并保留明确的辅助文本。

Provider：复用现有 `providerProfiles` 持久化表，加入 Provider 列表、添加、编辑、显式设为当前和删除。Repository 在事务内保证至多一个 active Provider；新建第二个及后续配置不会自动抢占当前 Provider。API Key 继续不写入 IndexedDB、日志或导出，按 Provider ID 隔离保存在当前标签页的 `sessionStorage`，刷新可恢复，切换 active Provider 时同步给模型调用层；旧单 Provider 会话密钥会在首次载入时迁移到对应 profile 的会话键。

真实界面：在 1278px 本地页面检查左右侧栏同时展开与同时收起，确认折叠按钮始终位于顶栏且画布无覆盖；检查 Provider 设置页已保存列表、当前状态和表单层级。Impeccable detector 对本次修改文件返回 0 项问题。

测试：Provider Repository 与会话密钥单元测试覆盖多配置、唯一 active 和按 profile 隔离；Provider 设置组件测试覆盖测试/保存分离；Playwright 新增保存两个 Provider、显式切换 active、刷新恢复非敏感字段和对应会话密钥的回归。完整 `npm run check` 结果见本阶段提交记录。

## v0.6.14 工作台与设置体验修正

实施日期：2026-09-11。本阶段根据真实界面批注调整工作台会话工具与设置页，不修改研究 Agent、图谱语义或 Provider 协议。

工作台：设置入口移到 IdeaScope 标题旁；“新建探索”统一改为“新建会话”；新增单会话档案导入图标按钮，并复用既有 `WorkspaceArchiveService` 的校验、迁移与非覆盖导入。会话搜索按需展开，可匹配会话标题和已持久化消息正文；中央节点搜索默认收为图标，点击后展开并聚焦输入框。中央导出继续只处理当前会话。

设置：修复来源/数据页使用不存在的 CSS token 导致输入框和分隔线无颜色的问题。Provider 当前状态改为紧凑五列摘要，能力探针和动作区层级更清晰。文献来源高级设置现在显示注册、启用、自定义来源统计，并承载 AI 配置助手与 Source/Pack 导入。数据与存储页改成唯一的“整体备份与恢复”入口，使用安全 Bundle 处理全部会话及非秘密资源；工作台侧栏继续承担单会话导入导出。

真实界面：在 1120px 本地工作台逐项检查设置、新建会话、导入会话、两类渐进搜索；检查 Provider 摘要、IEEE Key 输入边框、展开后的文献高级设置及整体备份页。Impeccable detector 仅报告一个既有 Context Chip 左侧 2px accent 警告，本次未改变该控件。

测试：`npm run check` 通过，包括 ESLint、严格 TypeScript、41 个 Vitest 文件 / 160 项测试、production build、9 项 Playwright E2E 与 secret scan。E2E 已更新为检查侧栏设置/导入/搜索入口、整体备份入口、折叠高级来源设置和“找回地图”真实路径。构建仍有既有约 2.24 MB 主 chunk 非阻断警告。

## v0.6.13 Research Map 视口恢复

实施日期：2026-09-11。本阶段只修复画布拖动/缩放后的视口稳定性，不扩展研究工作流。

根因：React Flow 的程序化 `fitView` / `setCenter` 结束事件与用户拖动共用 `onMoveEnd`，瞬态 viewport 可能被持久化；每次保存 viewport 又会克隆 Workspace，使依赖整个 Graph/View 对象的布局 effect 再次执行；旧回调还可能以空的 positions 覆盖已保存布局。三者叠加后会出现图被移到画面外且刷新仍恢复错误视口的情况。

修复：布局只在 Branch ID 或 Graph revision 真正变化时执行，不再被单纯 viewport 保存触发；只持久化有真实用户事件、有限数值且处于允许 zoom 范围内的 viewport；拒绝用空/非有限 positions 覆盖布局。加载时如果恢复的 viewport 无效或全部节点都在可见区域外，会自动执行一次 fit；工具栏新增“找回地图”，可明确恢复全图并保存恢复后的 viewport。

验证：新增 viewport 数值校验与节点可见性单元测试；真实 Chrome 中打开 22 节点/27 Evidence 的持久化会话，确认最新工具栏生效，并实际执行“找回地图”后全图恢复可见。完整自动化与生产构建结果见本阶段提交记录。
...
迭代闭环：新增 `searchAcademicIteratively`。Intent Plan 产生的 2～4 组查询不再在一次黑盒调用中全部并发，而是逐轮取下一组有差异的查询；每轮执行受预算的多来源并行搜索，观察累计去重候选是否达到充分阈值，再决定提前综合或继续下一轮。循环硬限制为最多 3 轮，同时受 `maxQueries` 和 `maxCandidates` 约束，不存在无限 Agent loop。

聚合：每轮 SearchRecord 和 source failure 独立保留；跨轮按既有 DOI/arXiv/外部 ID 规则再次去重，最后统一执行可解释排序。单轮失败不会清空前轮结果；达到候选预算立即停止。用户可展开的研究过程显示“第 N 轮检索完成”和累计候选数，但不暴露 Prompt 或隐藏推理。

测试：`npm run check` 通过，包括 ESLint、严格 TypeScript、31 个 Vitest 文件 / 135 项测试、production build、7 项 Playwright E2E 与 secret scan。新增充分后提前停止、按计划继续下一轮、三轮硬上限、候选预算及探索进度回归。构建仍有既有约 2.19 MB 主 chunk 非阻断警告。

下一阶段入口：v0.6.10-A，Source Assistant 只生成声明式、可校验的建议；任何安装都必须经过测试与用户确认，禁止执行未知代码。
